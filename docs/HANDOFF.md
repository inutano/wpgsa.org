...
参照ネットワークはマウス限定の 67 MB ファイルである。これらの刷新が第2段階になる。
...
単純な差し替えにはならない。現行は6列でウェイトを `positive No.` と `experiment No.` から
...
サイズも 67 MB から 1.2 GB になるため、t3.small（メモリ 2 GiB）では処理しきれない見込みで、
インスタンスタイプの見直しが前提になる。
