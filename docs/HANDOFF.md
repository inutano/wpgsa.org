...
最終更新: 2026-08-21 / 対象: `v1.0.0` + 参照ネットワークの列形式修正

第1段階（実行基盤の再構築）が完了し、本番へ反映済みである。
第2段階（解析実装と参照データの刷新）はまだ着手していない。

第1段階で参照ネットワークを6列のファイルに向けてしまい、
コンテナが列をずらして読む状態が 2026-08-18 から 2026-08-21 まで本番に出ていた。
5列の `merged_mouse_v7_160212.network` に差し替えて解消済みである。
この期間に生成された解析結果17件はホストから削除した。

## 現況

| 項目 | 値 |
|---|---|
| 直近のタグ | `v1.0.0`（`0d450f1`）。以降に参照ネットワークの列形式修正が入っている |
